...
var Database = class _Database {
  static {
    ...
    this.HTTP_METHOD_POST = HttpRequestMethod.Post;
  }
  static {
    this.HTTP_METHOD_GET = HttpRequestMethod.Get;
  }
  /**
  ...
  validatePlayerData(data) {
    if (!data?.id || typeof data.id !== "string" || data.id.trim().length === 0) {
      return { valid: false, error: "ID do jogador \xE9 obrigat\xF3rio e deve ser uma string n\xE3o vazia" };
    }
    if (!data?.nick || typeof data.nick !== "string" || data.nick.trim().length === 0) {
      return { valid: false, error: "Nick do jogador \xE9 obrigat\xF3rio e deve ser uma string n\xE3o vazia" };
    }
    if (!Array.isArray(data.inventory)) {
      return { valid: false, error: "Invent\xE1rio deve ser um array" };
    }
    for (let i = 0; i < data.inventory.length; i++) {
      const item = data.inventory[i];
      if (!item?.typeId || typeof item.typeId !== "string" || item.typeId.trim().length === 0) {
        return { valid: false, error: `Item no \xEDndice ${i}: typeId \xE9 obrigat\xF3rio e deve ser uma string n\xE3o vazia` };
      }
      if (typeof item.amount !== "number" || item.amount < 0 || !Number.isInteger(item.amount)) {
        return { valid: false, error: `Item no \xEDndice ${i}: amount deve ser um n\xFAmero inteiro n\xE3o negativo` };
      }
      if (typeof item.slot !== "number" || item.slot < 0 || !Number.isInteger(item.slot)) {
        return { valid: false, error: `Item no \xEDndice ${i}: slot deve ser um n\xFAmero inteiro n\xE3o negativo` };
      }
    }
    return { valid: true };
  }
  ...
  parseResponse(response) {
    try {
      ...
      throw new Error("Resposta da API n\xE3o \xE9 um JSON v\xE1lido");
    }
  }
  /**
  ...
  validateKey(key) {
    if (!key || typeof key !== "string" || key.trim().length === 0) {
      return { valid: false, error: "Chave inv\xE1lida. Deve ser uma string n\xE3o vazia." };
    }
    return { valid: true };
  }
  /**
  ...
  async save(key, data) {
    ...
    try {
      ...
      const request = this.createRequest(url, _Database.HTTP_METHOD_POST, JSON.stringify(data));
  ...
  async load(key) {
    ...
    try {
      const url = `${this.apiUrl}/api/${this.collectionName}/${encodeURIComponent(key.trim())}`;
      const request = this.createRequest(url, _Database.HTTP_METHOD_GET);
      const response = await http.request(request);
      if (response.status === 404) {
        return {
          sucesso: false,
          erro: "NOT_FOUND",
          mensagem: "Dados n\xE3o encontrados"
        };
      }
      if (response.status >= 200 && response.status < 300) {
        const responseData = this.parseResponse(response);
        if (!responseData.sucesso || !responseData.dados) {
          return {
            ...
            mensagem: "Resposta inv\xE1lida da API"
          };
        }
        ...
        if (!validation.valid) {
          return {
            sucesso: false,
            erro: "INVALID_DATA",
            mensagem: `Dados inv\xE1lidos recebidos: ${validation.error}`
          };
        }
        return {
          sucesso: true,
...
// scripts/utils/InventoryUtils.ts
import { ItemStack } from "@minecraft/server";

// scripts/constants/Messages.ts
var Messages = {
  // Mensagens de salvamento
  SAVING: "\xA77Salvando invent\xE1rio...",
  SAVE_SUCCESS: (itemCount) => `\xA7aInvent\xE1rio salvo com sucesso! (${itemCount} itens)`,
  SAVE_ERROR: (error) => `\xA7cErro ao salvar: ${error}`,
  // Mensagens de carregamento
  LOADING: "\xA77Carregando invent\xE1rio...",
  LOAD_SUCCESS: (itemCount) => `\xA7aInvent\xE1rio restaurado! (${itemCount} itens)`,
  LOAD_NOT_FOUND: "\xA7eNenhum invent\xE1rio salvo encontrado. Use um stick para salvar primeiro.",
  LOAD_ERROR: (error) => `\xA7cErro ao carregar: ${error}`,
  // Mensagens de erro
  INVENTORY_ERROR: "\xA7cErro: N\xE3o foi poss\xEDvel acessar o invent\xE1rio"
};

// scripts/utils/InventoryUtils.ts
var InventoryUtils = class {
  static {
    this.AIR_TYPE = "minecraft:air";
  }
  /**
  ...
  static restoreInventory(player, inventoryData) {
    ...
    for (const itemData of inventoryData) {
      if (itemData.slot < 0 || itemData.slot >= container.size) {
        console.warn(`Slot inv\xE1lido: ${itemData.slot} (m\xE1ximo: ${container.size - 1})`);
        continue;
      }
      if (!itemData.typeId || itemData.amount <= 0) {
        console.warn(`Item inv\xE1lido no slot ${itemData.slot}: typeId=${itemData.typeId}, amount=${itemData.amount}`);
        continue;
      }
      try {
...
var db = new Database(Config.COLLECTION_NAME, Config.API_URL);
async function savePlayerInventory(player) {
